...
class DefaultParameters(BaseModel):
        ...
        verbose: bool = Field(
            default=False,
            description="General application verbosity."
        )
        q_verbose: bool = Field(
            default=False,
            description="Queue construction verbosity."
        )
        medcam_enabled: bool = Field(
            default=False,
            description="Enable interpretability via medcam."
        )
        save_training: bool = Field(
            default=False,
            description="Save outputs during training."
        )
        save_output: bool = Field(
            default=False,
            description="Save outputs during validation/testing."
        )
        in_memory: bool = Field(
            default=False,
            description="Pin data to CPU memory."
        )
        pin_memory_dataloader: bool = Field(
            default=False,
            description="Pin data to GPU memory."
        )
        scaling_factor: int = Field(
            default=1,
            description="Scaling factor for regression problems."
        )
        q_max_length: int = Field(
            default=100,
            description="The max length of the queue."
        )
        q_samples_per_volume: int = Field(
            default=10,
            description="Number of samples per volume."
        )
        ...
        patience: int = Field(
            default=100,
            description="Number of epochs to wait for performance improvement."
        )
        ...
        print_rgb_label_warning: bool = Field(
            default=True,
            description="Print a warning for RGB labels."
        )
        data_postprocessing: Dict = Field(
            default={},
            description="Default data postprocessing configuration."
        )
        grid_aggregator_overlap: str = Field(
            default="crop",
            description="Default grid aggregator overlap strategy."
        )
        determinism: bool = Field(
            default=False,
            description="Enable deterministic computation."
        )
